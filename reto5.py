            ...
            while True:
                ...
                if 'contraseña' in miLista[opcion-1]:
                    ...
                    if menuZona >=1 and menuZona <=3:
                        ...
                        ubucacion_actual = coordenadas[menuZona -1].copy()
                ...
                elif 'Guardar archivo' in miLista[opcion-1]:
                    print("Usted ha elegido la opción",opcion)
                    result = True
                    m = 0
                    while result:
                    ...
                    if len(ubucacion_actual) == 0:
                        print("“Error de alistamiento")
                        exit() 

                    wifiCerca = {"actual":ubucacion_actual,"zonaswifi1":[corDistancia[0][0:3]],"recorrido":[corDistancia[0][3],"moto",tiempoMoto]}
                    print(wifiCerca)
                    findeprog = False
                    saliramenu = False
                    while True:
                        respuesta = input("¿Está de acuerdo con la información a exportar? Presione 1 para confirmar, 0 para regresar al menú principal")
                        if respuesta == "1": #si
                ...
                elif 'Actualizar registros' in miLista[opcion-1]:
                    print("Usted ha elegido la opción",opcion)

                    newdata = open("./matrizUbicación.csv","r")
                    newDestinos = []
                    for line in newdata.readlines():
                        lista = line.rstrip().split(',')

                        newLat = float(lista[0])
                        newLon = float(lista[1])
                        newusers = lista[2]
                        newDestinos.append([newLat, newLon, newusers])    
                    newdata.close()
                    posiblesdestinos=newDestinos

                    while True:
                        vali=input("Datos de coordenadas para zonas wifi actualizados, presione 0 para regresar al menú principal\n")
                        if vali == "0":
                            os.system ("cls")
                            break
